...
from apps.shops.models import Post, Category, Tag
...
class Command(BaseCommand):
    ...
    def parse(self, date):
        ...
        for news in news_list:
            try:
                self.parse_article(news)
            except Exception as e:
                self.stdout.write(self.style.ERROR(_(f"Error: {e}")))

    def parse_article(self, news):
        news_url = news.find('a', {'class': 'ArticleItem--name'}).get('href')
        post = Post.objects.filter(origin_url=news_url).first()

        if post:
            self.stdout.write(self.style.SUCCESS(_(f"News already exist - {news_url}")))
            return
